...
@export_range(16, 32, 2) var tesselation_size: int = 24 :
	set(value):
		tesselation_size = clamp(value, 16, 32)
		_generate_clipmap()
...
func _update_mesh_size(size: int) -> void:
	...
	for x in size:
		for y in size:
			var pos: Vector3 = Vector3(x * 2 - size, 0, y * 2 - size)
...
func _generate_clipmap() -> void:
	...
	_array_meshs.append(_generate_mesh(Vector2i(2, 2), tesselation == Tesselation.DISABLED && symetric_mesh
		or tesselation != Tesselation.DISABLED, max(pow(2,tesselation), 1)))
	_array_meshs[HD_TILE].surface_set_material(0, details_mat)
	_clear_clipmap_rids()
	...
	for level in lod_levels + 1:
		...
		if level == 0:
			...
			aabb = _array_meshs[HD_TILE].get_aabb()
			aabb.position.y = -cull_margin
			aabb.size.y = cull_margin * 2.0
			for i in hd_tile_pos.size():
				# Tiles outside the Tesselation area are populated with TABs.
				if hd_tile_pos[i].length() < min(tesselation_size, mesh_size) and tesselation > 0:
	...
	details_mat.set_shader_parameter("mesh_size", min(tesselation_size, mesh_size) - 4)
...
func _process(_delta: float) -> void:
	var active_cam: Camera3D = cam
	if Engine.is_editor_hint() == true:
		active_cam = EditorInterface.get_editor_viewport_3d(0).get_camera_3d()
	...
	if active_cam.global_position.distance_to(last_cam_pos) > 1.0 or update:
		...
		for lod in _lod_rids.size():
			...
			var lod_scale: Vector3 = Vector3(pow(2, lod), 1, pow(2, lod))
			pos.x = roundf(active_cam.global_position.x / snap_step) * snap_step
			pos.z = roundf(active_cam.global_position.z / snap_step) * snap_step
			# Reposition edge strips and corner tabs [0, 1, 2]
			var test_x: int = clampi((pos.x - roundf(active_cam.global_position.x
				/ (snap_step * 2.0)) * snap_step * 2.0) / snap_step + 1, 0, 2)
			var test_z: int = clampi((pos.z - roundf(active_cam.global_position.z
				/ (snap_step * 2.0)) * snap_step * 2.0) / snap_step + 1, 0, 2)
				...
				match mesh:
					...
					HD_TILE:
						for instance in _lod_rids[lod][HD_TILE].size():
							...
							## TODO HD tiles could have much smaller vertical AABB, and be positioned on the Y-axis
							## for better culling optimization, as they are much more expensive due to high vertex count.
							#t.origin.y = heightmap value
