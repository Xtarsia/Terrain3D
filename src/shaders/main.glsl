...
#define SQRT2_HALF 0.707106781186547 // sqrt(2.) / 2.
...
// i_data constructor
#define I_DATA(control) i_data(ivec2(DECODE_BASE(control), DECODE_OVER(control)), vec2(0.), DECODE_BLEND(control))
...
float accumulate_weight(int texture_id, i_data data[4], vec4 weights) {
	float w = 0.0;
	for (int i = 0; i < 4; i++) {
...
void accumulate_material(vec3 base_ddx, vec3 base_ddy, float weight, ivec3 index, i_data data,
		...
		if (i_normal.y <= projection_threshold && enable_projection) {
			...
			vec3 p_tangent = cross(p_normal, vec3(0., 1., 0.));
			vec3 p_binormal = cross(p_tangent, p_normal);
			p_angle = atan(-i_normal.x, -i_normal.z);
			// Project uv_center for detiling
			vec3 p_pos = vec3(i_pos.x, h, i_pos.y);
			i_pos = floor(vec2(dot(p_pos, p_tangent), dot(p_pos, p_binormal)));
			i_uv = vec2(dot(v_vertex, p_tangent), dot(v_vertex, p_binormal));
			i_dd.xy = vec2(dot(base_ddx, p_tangent), dot(base_ddx, p_binormal));
			i_dd.zw = vec2(dot(base_ddy, p_tangent), dot(base_ddy, p_binormal));
		}

		// Control map scale & rotation
		float i_scale = DECODE_SCALE(control);
		i_dd *= i_scale;
		i_uv *= i_scale;
		i_pos *= i_scale;
		float i_angle = DECODE_ANGLE(control);
		i_uv = rotate_around(i_uv, vec2(0), i_angle);
		i_pos = rotate_around(i_pos, vec2(0), i_angle);
...
//INSERT: DUAL_SCALING_MIX
			...
			float id_weight = exp2(sharpness * log2(weight + id_w + alb.a * world_normal));
...
//INSERT: AUTO_SHADER
	...
	float sharpness = fma(24., blend_sharpness, 8.);
	
	// 2 - 4 lookups, 2 - 6 if dual scale texture
	accumulate_material(base_ddx, base_ddy, weights[3], index[3], data[3], index_normal[3], h[3], control[3],
		TANGENT_WORLD_MATRIX, sharpness, albedo_height, normal_rough, normal_map_depth, ao_strength, total_weight);

	// 6 - 12 lookups, 6 - 18 if dual scale texture
	if (bilerp) {
		accumulate_material(base_ddx, base_ddy, weights[2], index[2], data[2], index_normal[2], h[2], control[2],
			TANGENT_WORLD_MATRIX, sharpness, albedo_height, normal_rough, normal_map_depth, ao_strength, total_weight);
		accumulate_material(base_ddx, base_ddy, weights[1], index[1], data[1], index_normal[1], h[1], control[1],
			TANGENT_WORLD_MATRIX, sharpness, albedo_height, normal_rough, normal_map_depth, ao_strength, total_weight);
		accumulate_material(base_ddx, base_ddy, weights[0], index[0], data[0], index_normal[0], h[0], control[0],
			TANGENT_WORLD_MATRIX, sharpness, albedo_height, normal_rough, normal_map_depth, ao_strength, total_weight);
	}
